# Portus Documentation

## Product Docs (Operators & Support)
- [Overview](product/portus-overview.md) — What Portus does, access control, export modes, modules

## Engineering Docs (Developers & Agents)
- [Developer Guide](engineering/developer-guide.md) — Architecture, module system, export/import pipelines, extension points
- [Manifest Reference](engineering/manifest-reference.md) — Full manifest envelope and payload shapes
- [Access Control](engineering/access-control.md) — Domain-based access gate
- [Add Module Playbook](engineering/add-module-playbook.md) — Step-by-step walkthrough for adding a new module

## Guides (End Users)
- [Export and Import Configuration](guides/portus-user-guide.md) — Step-by-step export/import flow for operators

## Planning History (Archive)
- [Hackweek Plan](guides/project-portus-hackweek-plan.md) — Historical planning artifact; not runtime documentation
- [Accelerated Execution Plan](guides/project-portus-accelerated-execution-plan.md) — Historical planning artifact; not runtime documentation